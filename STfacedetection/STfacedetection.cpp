...
cv::CascadeClassifier g_cascade("D:\\Programming_Project\\OpenCV\\StopTruancy\\x64\\Debug\\haarcascade_frontalface_alt.xml");	// 默认采用haar的人脸集
...
bool ClassifierDetectEx(void* pMat, int flag, int minx, int miny, int maxx, int maxy) {
	if (g_cascade.empty() || !pMat) return false;
	cv::Mat origin = static_cast<cv::Mat*>(pMat)->clone();
	cv::cvtColor(origin, g_gray, CV_BGR2GRAY);	// 转灰度图
	cv::equalizeHist(g_gray, g_gray);	// 直方图均衡化，简单点来说就是提高对比度
	g_cascade.detectMultiScale(g_gray, g_faceRect, 1.1f, 3, flag, cv::Size(minx, miny), cv::Size(maxx, maxy));
	return true;
